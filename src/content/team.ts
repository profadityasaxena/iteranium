import type { TeamMember } from "@/types/content";

export const teamMembers: TeamMember[] = [
  // ── Directors ──────────────────────────────────────────────
  {
    name: "Tarek Sabra",
    role: "Director & Chief Strategist | Market Strategy & Growth",
    tier: "director",
    bio: "Serves as Iteranium's Chief Strategist, driving market positioning, go-to-market strategy, and strategic partnerships. He brings deep expertise in market strategy, client engagement, and commercialization of complex technology solutions, ensuring Iteranium's offerings are clearly differentiated, market-aligned, and scalable across industries and regions.",
    image: "/images/profiles/tarek.png",
  },
  {
    name: "Vineet Saxena",
    role: "Director | Industry 4.0 & Smart Manufacturing",
    tier: "director",
    bio: "A senior Industry 4.0 leader with over 35 years of experience delivering large-scale industrial automation and digital manufacturing programs. He has held leadership roles at organizations such as General Electric and Rockwell Automation, leading complex OT–IT integration initiatives across automotive, electronics, life sciences, and industrial manufacturing. Vineet brings deep expertise in MES/MOM, industrial automation platforms, and enterprise manufacturing modernization, and is widely recognized for translating manufacturing strategy into scalable, production-ready systems.",
    image: "/images/profiles/vineet.png",
  },
  {
    name: "Bhavik Gandhi",
    role: "Director | Data Science & Machine Learning",
    tier: "director",
    bio: "Oversees Iteranium's data science and machine learning capabilities, translating advanced analytics into practical manufacturing intelligence. He brings strong experience in machine learning, predictive analytics, and applied AI, including work across enterprise analytics platforms. Bhavik leads the development of models and analytics frameworks that enable predictive quality, optimization, and operational intelligence across Iteranium's solutions.",
    image: "/images/profiles/bhavik.png",
  },
  {
    name: "Aditya Saxena",
    role: "Director | Technology, AI & Solution Architecture",
    tier: "director",
    bio: "Leads Iteranium's technology strategy and solution architecture, with deep hands-on experience in artificial intelligence, machine learning, and explainable AI. He combines this with strong expertise in cloud computing, full-stack development, IIoT architectures, industrial data platforms, and data engineering. Aditya is responsible for designing secure, scalable, and interpretable manufacturing intelligence systems—ensuring AI-driven decisions are transparent, auditable, and production-ready across MES, IIoT, and enterprise environments.",
    image: "/images/profiles/aditya.png",
  },

  // ── Principals ─────────────────────────────────────────────
  {
    name: "Abbey Ojo",
    role: "Principal | Enterprise ERP & MIS Systems",
    tier: "principal",
    bio: "Abbey brings over 25 years of experience in enterprise systems, manufacturing ERP, and large-scale business system integration. He has led and supported ERP modernization initiatives across production planning, MRP, inventory management, payroll, finance, and operational reporting environments. With deep experience across platforms including Oracle, Microsoft Dynamics, Sage, and other enterprise ecosystems, Abbey strengthens Iteranium's end-to-end Industry 4.0 capability—ensuring tight integration between manufacturing systems, finance, workforce management, and enterprise architecture. He plays a key role in aligning ERP modernization with smart manufacturing and digital transformation objectives.",
    image: "/images/profiles/abbey.png",
  },
  {
    name: "Garima Malik",
    role: "Principal | Machine Learning & Artificial Intelligence",
    tier: "principal",
    bio: "Leads advanced AI research and applied machine learning initiatives across Iteranium's Industry 4.0 programs. Garima brings strong expertise in natural language processing, scalable machine learning systems, and reinforcement learning applications for industrial environments. She supports both research-driven innovation and production-grade AI solution design, strengthening Iteranium's full-stack capabilities across manufacturing intelligence, predictive systems, and next-generation automation frameworks.",
    image: "/images/profiles/garima.png",
  },

  {
    name: "Shraddha Padwekar",
    role: "Principal | Digital Transformation Delivery",
    tier: "principal",
    bio: "Supports the delivery of digital transformation initiatives for Industry 4.0, working across IIoT, MES, and manufacturing intelligence programs. She brings hands-on experience in coordinating cross-functional teams, translating business requirements into execution plans, and supporting structured delivery of smart manufacturing solutions. Shraddha plays a key role in ensuring alignment between technology implementation and operational outcomes.",
    image: "/images/profiles/shraddha.png",
  },

  // ── Managers ───────────────────────────────────────────────
  {
    name: "Avneet Hazooria",
    role: "Manager | Project Management & Operations",
    tier: "manager",
    bio: "Responsible for project governance, coordination, and operational execution across Iteranium engagements. With a strong background in project management and administrative operations, Avneet ensures disciplined delivery, effective risk management, and consistent execution standards across complex Industry 4.0 and manufacturing AI initiatives.",
    image: "/images/profiles/avneet.png",
  },
];
